...
package com.dc.util.pds.executor.ordered;
 ...
 * processing response of other partitions. It also maintains the order for within each partition, to ensure the messages are handled
 * in the order received.
 */
public class PartitionOrderedExecutorService<M> extends AbstractPartitionExecutorService<M> {
    ...
    public PartitionOrderedExecutorService(PartitionMessageHandler<M> messageHandler, Configuration configuration) throws IllegalArgumentException {
    ...
    private void initialize() {
        ...
        if(configuration.isImplicitClose()) {
            threadPoolExecutor = new ThreadPoolExecutor(configuration.getMinNoOfThreads(), configuration.getMaxNoOfThreads(),
                configuration.getIdleThreadCleanupInSecs(), TimeUnit.SECONDS, executorServiceQueue, new DaemonThreadFactory("PartitionedExecutorPooledThread-" + id));
        }
        else {
            threadPoolExecutor = new ThreadPoolExecutor(configuration.getMinNoOfThreads(), configuration.getMaxNoOfThreads(),
                configuration.getIdleThreadCleanupInSecs(), TimeUnit.SECONDS, executorServiceQueue);
        }
            ...
            while(!done) {
                ...
                try {
                    ...
                    if(partitionID != null) {
                        ...
                        if(addToService) {
                            queueWorker = new PartitionExecutorWorker<M>(messageHandler, partitionID, executorServiceQueue,
                                    pendingExecutorServiceQueue, configuration, messagesMap, messageEnqueuedCountMap, currentMessageCount);

                            executorCompletionService.submit(queueWorker);
                            //System.out.println(Thread.currentThread().getName() + " | Added : " + partitionID);
                        } //else do nothing
                        else {
                            //System.out.println(Thread.currentThread().getName() + " | Not Added : " + partitionID);
                        }
                    } //else do nothing
                }
    ...
    private void handlePendingWork() {
        ...
        System.out.println(Thread.currentThread().getName() + " | Available count : " + availableCount);
        if(availableCount > 0) {
    ...
    private boolean incrementPartitionMessageCount(String partitionID) {
        ...
        synchronized(partitionID.intern()) {
            ...
            //System.out.println(Thread.currentThread().getName() + " | message count : " + messageCount);
